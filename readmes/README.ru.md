...
Этот репозиторий публикует Docker-образ для серверов EaglercraftX. Образ
...
| --- | --- | --- |
| `MINECRAFT_VERSION` | Да | Выбирает `1.8` или `1.12`. |
| `RCON_PASSWORD` | Нет | Включает RCON и admin API на порту `5201`. |
| `SERVER_DATA_DIR` | Нет | Устаревший путь постоянного хранения только для миров. |
| `ADMIN_AUTH_SECRET` | Нет | Переопределяет секрет подписи admin-токена. |
| `ADMIN_AUTH_TOKEN_TTL` | Нет | Задает срок жизни admin-токена в секундах. |
| `DYNMAP_HOST` | Нет | Необязательный хост прокси Dynmap. По умолчанию: `127.0.0.1`. |
| `DYNMAP_PORT` | Нет | Необязательный порт прокси Dynmap. По умолчанию: `8123`. |
